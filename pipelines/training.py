...
from models.model import MultiTargetModel
...
def evaluate(
    ...
    for quantile in quantiles:
        for horizon in horizons:
            true = df_test[f"next_{horizon}d"].values
            pred = df_pred[f"pred_{horizon}d_q{int(quantile*100)}"].values
            loss = evaluate_model(true, pred, quantile)
...
def run_pipeline(
    ...
    model = fit_model(df_train, model_features, quantiles, horizons)
...
def main(
    orders_url: str = "https://disk.yandex.ru/d/0I0lrw6t3cgo3w",
    ...
    Args:
        ...
        model_path (str): Local path to save the production model.
        debug (bool, optional): Run the pipeline in debug mode. Defaults to False.
    """
